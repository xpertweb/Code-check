	...
	<div class="portlet-body">
		<div class=''>
			<div class='col-sm-4 pull-left'>
				...
				@if($filesCount != 0)
					<a href="{{ route('admin-downloadZip',$library_id) }}" class="btn btn-success" >@lang('messages.download')</a>
				@endif
			</div>
		</div>
			    ...
			    @if($filesCount != 0)
					@foreach($files as $file)	
					  <tr>
						...
						<td>{{!empty($file->createuser->name)?$file->createuser->name:''}}</td>
						<td>{{($file->original_name)}}</td>
						<td>{{strtoupper($file->file_type)}}</td>
						<td>{{date('M d Y',strtotime($file->created_at))}}</td>					
						<td><a class='btn btn-danger' onclick="return confirm('Are you sure you want to delete selected ?')" href="{{ route('admin-delete-file',$file->file_id) }}"><i class='fa fa-trash'></i></a></td>
					  </tr>	
					@endforeach
				@endif
			</tbody>
