...
<div class="row">
	<div class="col-md-12">
		...
		<form class="leadsform" action="{{ route('admin-add-leads') }}" method="post">
			<input type="hidden" name="leads_type" value="1" />
			{{ csrf_field() }}
					...
					<div class="col-sm-8">
						<select class="form-control" name="designer_id" required >
							...
							@foreach($user as $users)
								<option @if(old("designer_id") == $users->designer_id) selected @elseif($users->designer_id==166) selected @endif value="{{$users->designer_id}}">{{$users->name}}</option>
							@endforeach
						</select>
					</div>
				</div>
