@extends('layouts.app')
@section('title', 'Tilføj nyt projekt')
@section('content')

<div class="row">
	<div class="col-md-12">
		<!-- BEGIN SAMPLE FORM PORTLET-->
		<form class="projectform" action="{{ route('admin-add-project') }}" method="post">
			{{ csrf_field() }}
			<input type='hidden' name='projectAssigned' class='project_Assigned' value="{{old('project_Assigned')}}" />
			<input type='hidden' name='projectCapacity' class='project_Capacity' value="{{old('projectCapacity')}}" />
			<input type='hidden' name='projects_type' value="0" />
			<div class="portlet box blue">
				<div class="portlet-title">
					...
					<div class="col-sm-4 text-center" style="margin-top: 3px;">
						...
						<div class="col-sm-8">
							<select class="form-control selectDesigner" name="designer_id" required >
								<option value="">@lang('messages.designerName')</option>
								@foreach($user as $users)
									<option @if(old("designer_id") == $users->id) selected  @elseif($users->id==166) selected @endif value="{{$users->id}}">{{$users->name}}</option>
								@endforeach
							</select>
						</div>
					</div>
						...
						<div class="row">
							<div class="col-sm-6">
								...
								<div class="input-group">
									...
									<input type="text" name='project_name' class="form-control" value="{{old('project_name')}}" placeholder="@lang('messages.proName')">
								</div>
							...
							<div class="col-sm-6">
								...
								<div class="input-group">
									...
									<input type="text" name='client_email' class="form-control" value="{{old('client_email')}}" placeholder="@lang('messages.cemail')">
								</div>
							...
							<div class="col-sm-6">
								...
								<div class="input-group">
									...
									<input type="text" name='client_address' class="form-control" value="{{old('client_address')}}" placeholder="@lang('messages.client_address')">
								</div>
							...
							<div class="col-sm-6">
								...
								<div class="input-group">
									...
									<input type="text" name='project_link' class="form-control" value="{{old('project_link')}}" placeholder="@lang('messages.link')">
								</div>
