...
<div class="row">
	<div class="col-md-12">
		...
		<form class="projectform" action="{{ route('add-project') }}" method="post">
			{{ csrf_field() }}
			...
			<input type='hidden' name='projects_type' class='projects_type' value="0" />
						...
						<div class="col-sm-8">
							<select class="form-control selectDesigner" name="designer_id" >
								...
								@foreach($user as $users)
									<option @if($designer_id == $users->id) selected  @endif value="{{$users->id}}">{{$users->name}}</option>
								@endforeach
							</select>
						</div>
					</div>
...
<script>
	var select_designer = "{{ route ('select-designer')}}";	
</script>
