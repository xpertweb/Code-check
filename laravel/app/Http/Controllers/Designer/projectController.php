	...
	public function completeUpdate(Request $request){
		...
		if(!empty($request->id)){
			...
			try
			{
				...
				if($MailHelper)
				{
					return redirect('/')->with("success", "Project completed successfully. Mail Send to admin.");
				}
				else
				{
					return redirect('/')->with("success", "Project completed successfully. Somthing went wrong to send an email.");
				}
			}
			catch (\Exception $e)
			{
