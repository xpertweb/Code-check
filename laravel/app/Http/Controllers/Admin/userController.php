...
class userController extends Controller
{
    ...
    {
		$data['user'] = User::with('designer')->withCount('processProject')->where(['is_deleted' => 0 , 'role_id' => 2])->get();
	...
	public function add(Request $request)
    {
		if (!empty($request->post()))
		{
			...
			if(!empty($user_id)){
				...
				return back()->with("success", "Designer inserted successfully.");
			}else{
    ...
    {		
		if (!empty($request->post()))
		{
			...
			return back()->with("success", "Designer information updated successfully.");		
    ...
    {		
		if (!empty($request->id))
		{
			...
			return redirect()->back()->with("error", "Designer deleted successfully.");
		}
    }	
    ...
    {
		$data['user'] = User::with('designer')->where("role_id", "=", 2)->where("status", "=", 1)->get();
        return view("admin.user.active",$data);
    }

	public function inactive()
    {
		$data['user'] = User::with('designer')->where("role_id", "=", 2)->where("status", "=", 2)->get();
        return view("admin.user.inactive",$data);
    }
	
	public function upgrade(Request $request){
		
		User::where("id",$request->id)->update(['role_id' => 3]);
		return back()->with("success", "Designer upgrade to admin successfully.");
